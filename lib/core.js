...
function countUsage(agg, session, got, opts, agent) {
  ...
  const provider = got.provider || (got.model ? agg.modelProviders.get(got.model) || null : null);
  ...
  if (got.api) {
    const a = agg.apiBreakdown.get(got.api) || { calls: 0, tokens: 0 };
    a.calls += 1;
    a.tokens += got.usage.totalTokens;
    agg.apiBreakdown.set(got.api, a);
  }
...
function processSessionFile(info, agg, opts, seen, seenSwitch, agent) {
  ...
  for (const line of text.split('\n')) {
    ...
    if (entry.type === 'model_change') {
      ...
      if (kind !== 'subagent' && entry.id && !seenSwitch.has(entry.id)) {
        seenSwitch.add(entry.id);
        agg.modelSwitches += 1;
      }
      continue;
    }
    ...
    // Record provider for this model id, so provider-less transcript entries
    // can be attributed to the right provider.
    if (got.model && got.provider) agg.modelProviders.set(got.model, got.provider);

    // Fork dedupe: shared history carries the same entry ids.
